...
def move_until_contact(rtde_c, rtde_r, xd, direction, marge_force=8.0,
    ...
    """
    Descend jusqu'au contact. Le seuil de detection est calcule
    automatiquement a partir de plusieurs mesures du residu au repos
    (moyenne + ecart-type), plutot qu'une seule lecture ponctuelle — plus
    robuste si le bruit du capteur fluctue d'une mesure a l'autre.
 
    seuil = moyenne(residu) + max(marge_force, 3 * ecart_type(residu))
 
    Le terme en ecart-type garantit que le seuil suit le bruit reel du
    capteur (evite les faux positifs si le bruit est plus grand que
    marge_force) ; marge_force reste un plancher minimal.
    """
    rtde_c.zeroFtSensor()
    time.sleep(0.5)
...
def prendre_support_L298N(rtde_c, rtde_r, data):
    ...
    pince_open(width=42, force=80, speed=100)
